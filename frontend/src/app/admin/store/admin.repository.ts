...
export class AdminRepository {
  ...
  setPriority(id: number, ticket: TicketAdminUpdateRequest): Observable<TicketResponse> {
    return this.http.put<TicketResponse>(`${this.adminServiceUrl}/${id}/priority`, ticket);
  }

  setStatus(id: number, ticket: TicketAdminUpdateRequest): Observable<TicketResponse> {
    return this.http.put<TicketResponse>(`${this.adminServiceUrl}/${id}/status`, ticket);
  }

  setTag(id: number, ticket: TicketAdminUpdateRequest): Observable<TicketResponse> {
  ...
  createStatus(request: StatusRequest): Observable<StatusResponse> {
    return this.http.post<StatusResponse>(`${this.adminServiceUrl}/statuses`, request);
  }

  createTag(request: TagRequest): Observable<TagResponse> {
    return this.http.post<TagResponse>(`${this.adminServiceUrl}/tags`, request);
  }
}